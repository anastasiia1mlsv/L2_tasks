...
func routeUp(currentRoute string) string {
	...
	//билдим наш путь в единую строку из массива
...
func (p *proc) getAndLogEnv() {
	...
	if err != nil {
		fmt.Println("Error at parsing environment:", err)
		return
	}
	for _, obj := range all {
		fmt.Println(obj)
...
func killProcById(Pid int) error {
	...
	if errFind != nil {
		return fmt.Errorf("failed to find process with PID %d: %v", Pid, errFind)
	}

	// Пытаемся завершить процесс
	errKill := proc.Kill()
	if errKill != nil {
		return fmt.Errorf("failed to kill process with PID %d: %v", Pid, errKill)
	}
