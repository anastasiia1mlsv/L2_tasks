...
func main() {
	...
	// Буфер для вводимых в консоль данных
	...
	for {
		...
		// Отделяем нулевой элемент вводимой строки от всех остальных, это будет команда,
		// в нашем случае либо sort, либо exit. Также триммируем суффиксы для получения
		// в конечной команды в виде строки
...
func (inp *input) parseInput() {
	...
	//для наилучшего взаимодействия проинициализируем глобальный путь к исполняемому файлу
	path, err := os.Getwd()
	if err != nil {
		log.Println(err)
	}
	...
	defer func(cont *os.File) {
		err := cont.Close()
		if err != nil {
			fmt.Printf("Error at closing file: %v\n", err)
		}
	}(cont)
	...
	case "-k":
		...
		//конвертированные данные из исходного массива строк файла
...
// ReadFile - чтение файла и возврат данных
