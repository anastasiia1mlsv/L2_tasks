...
func main() {
	...
	for {
		...
		//создаем объект скачивания и качаем его
		ob := object{
			href:   href,
			params: params,
		}
...
func (o *object) download() error {
	...
	//буфер обмена,
	//последние 2 параметра необходимы для расчета скорости скачивания
	...
	for {
		//читаем буфер целевого файли и пишем в наш
		...
		//если буфер для скачки не пустой - продолжаем качать
		if n > 0 {
			downloadedBytes += n
